...
// Capabilities that support transition duration (when the Z2M device has the transition option)
...
export default abstract class Zigbee2MQTTDevice extends Homey.Device {
  ...
  async setCapabilityUnits() {
    ...
    const supportsTransition = this.deviceSupportsTransition();
    ...
    for (const { cap: homeyCapability, expose } of allMappings) {
      ...
      if (supportsTransition && DURATION_CAPABILITIES.includes(homeyCapability)) {
  ...
  private deviceSupportsTransition(): boolean {
    const deviceInfo = this.getDeviceInfo();
    if (!deviceInfo) return false;
    const device = deviceInfo.type === 'group' ? deviceInfo.devices[0] : deviceInfo.device;
    return device?.definition?.options?.some((opt) => opt.name === 'transition') ?? false;
  }

  private static durationToTransition(durationMs: unknown): number | undefined {
    if (typeof durationMs !== 'number' || durationMs <= 0) return undefined;
    return Math.round(durationMs / 100) / 10; // round to 0.1s precision
  }

  async restartDevice(delay: number = 1000 * 5) {
    ...
    this.log(`Device will restart in ${delay / 1000} seconds`);

    await setTimeoutPromise(delay);
    await this.onInit().catch((error) => this.error(error));
  }
  ...
  async getStatus(payload: any, source: string) {
    if (!this.isConnected) throw new Error('Bridge is not connected');
  ...
  async setCommand(payload: any, source: string) {
    if (!this.isConnected) throw Error('Bridge is not connected');
  ...
  // ============================================================================
  // Bridge Connection and Message Handling Methods
  // ============================================================================

  private handleBridgeMessage: ((topic: string, message: any) => void) | null = null;
  private subscribeBridgeTopics: (() => void) | null = null;

  async connectBridge() {
    if (this.bridge) this.removeBridgeListeners();
  ...
  private removeBridgeListeners() {
    if (!this.bridge?.client) return;
    if (this.subscribeBridgeTopics) this.bridge.client.removeListener('connect', this.subscribeBridgeTopics);
    if (this.handleBridgeMessage) this.bridge.client.removeListener('message', this.handleBridgeMessage);
  }
  ...
  private async handleDeviceStateMessage(z2mState: Z2MState) {
    for (const [z2mProperty, z2mValue] of Object.entries(z2mState)) {
      ...
      for (const [capName, capVal] of Object.entries(capValues)) {
        ...
        if (capVal === undefined || capVal === null) continue;
        ...
        if (capName === 'action') {
          if (capVal !== null && capVal !== '') {
            ...
            this.homey.flow
              .getDeviceTriggerCard('action_event_received')
              .trigger(this, tokens, state)
              .catch(this.error);
          }
          await this.setCapability(capName, '---').catch(this.error);
        }

        await this.setCapability(capName, capVal).catch(this.error);
      }
    }
  }
