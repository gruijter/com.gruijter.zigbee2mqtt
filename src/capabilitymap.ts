...
export const mapProperty = function mapProperty(Z2MDevice: any) {
  const caps: string[] = [];
  const capDetails: any = {};
  if (Z2MDevice.definition && Z2MDevice.definition.exposes) {
    // console.log(Z2MDevice.definition.exposes);
    Z2MDevice.definition.exposes.forEach((expose: any) => {
      // if (expose.features) console.log(expose.features);
      if (expose.features) {
        expose.features.forEach((feature: any) => {
          if (capabilityMap[feature.property]) {
            const cap = capabilityMap[feature.property]()[0];
            if (!caps.includes(cap)) {
              caps.push(cap);
              capDetails[cap] = feature;
            }
          }
        });
      }
      if (capabilityMap[expose.property]) {
        const cap = capabilityMap[expose.property]()[0];
        if (!caps.includes(cap)) {
          caps.push(cap);
          capDetails[cap] = expose;
        }
      }
    });
  }
...
export const mapClassIcon = function mapClassIcon(Z2MDevice: any) {
  let homeyClass = 'other';
  let icon = '/icons/zigbee.svg';
  if (Z2MDevice.definition && Z2MDevice.definition.description) {
    const description = Z2MDevice.definition.description.toLowerCase();
