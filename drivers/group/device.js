...
class MyDevice extends Device {
	...
	async onUninit() {
		this.log('Group unInit', this.getName());
		this.destroyListeners();
		await setTimeoutPromise(2000);	// wait 2 secs
	}

	async onAdded() {
		this.log('Group added', this.getName());
	...
	async onDeleted() {
		...
		this.log('Group deleted', this.getName());
	}
	...
	async restartDevice(delay) {
		...
		this.log(`Group will restart in ${dly / 1000} seconds`);
		// this.setUnavailable('Group is restarting');
	...
	async migrate() {
		try {
			this.log(`checking group migration for ${this.getName()}`);
			...
			for (let index = 0; index < correctCaps.length; index += 1) {
				...
				if (caps[index] !== newCap) {
					this.setUnavailable('Group is migrating. Please wait!').catch(this.error);
					capsChanged = true;
	...
	async getStatus(payload, source) {
		...
		if (this.store && this.store.dev && this.store.dev.power_source === 'Battery') return Promise.resolve(true); // skip battery groups
	...
	async setCommand(payload, source) {
		...
		// check if ENUM command is supported by this group
	...
	async checkChangedOrDeleted() {
		if (!this.bridge || !this.bridge.groups) return;
		const [groupInfo] = this.bridge.groups.filter((dev) => dev.id === this.settings.uid);
		// check deleted
		if (!groupInfo) {
			this.error('group was deleted in Zigbee2MQTT', this.settings.friendly_name);
			this.setUnavailable('group went missing in Zigbee2MQTT').catch(this.error);
			throw Error('group went missing in Zigbee2MQTT');
		}
		// check for name change
		if (groupInfo.friendly_name !== this.settings.friendly_name) {
			this.log('group was renamed in Zigbee2MQTT', this.settings.friendly_name, groupInfo.friendly_name);
			this.setSettings({ friendly_name: groupInfo.friendly_name }).catch(this.error);
			this.restartDevice(1000).catch(this.error);
		}
	}
	...
	async connectBridge() {
		try {
			...
			if (bridgeDriver.getDevices().length < 1) throw Error('The source bridge group is missing in Homey.');
			const bridge = bridgeDriver.getDevice({ id: this.settings.bridge_uid });
			if (!bridge || !bridge.client) { throw Error('Cannot connect to source bridge group in Homey.'); }
			this.bridge = bridge;
			...
			const subscribeTopics = async () => {
				try {
					...
					await this.bridge.client.subscribe([`${this.deviceTopic}`]); // group state updates
	...
	async registerHomeyEventListeners() {
		...
		this.eventListenerGroupListUpdate = async () => {
			// console.log('grouplist update event received');
	...
	registerListeners() {
		try {
			...
			capArray.forEach((map) => {
				const mapFunc = map[1];
				if (mapFunc().length > 2 && this.getCapabilities().includes(mapFunc()[0])) {	// capability setting is mapped and present in group
