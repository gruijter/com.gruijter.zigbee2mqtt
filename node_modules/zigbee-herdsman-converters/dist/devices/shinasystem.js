...
const fzLocal = {
    poll_summ_received_on_electrical_measurement: {
        ...
        convert: (model, msg, publish, options, meta) => {
            (0, node_assert_1.default)(["PMM-300Z3", "PMM-300Z2"].includes(msg.device.modelID), "Poll summ received converter not supported for device");
            if ((msg.device.modelID === "PMM-300Z3" && msg.device.applicationVersion >= 9) ||
                (msg.device.modelID === "PMM-300Z2" && msg.device.applicationVersion >= 8)) {
                if (msg.data.rmsCurrent) {
                    msg.endpoint
                        .read("seMetering", ["currentSummReceived"])
                        .catch((error) => logger_1.logger.debug(`Failed to poll currentSummReceived of '${msg.device.ieeeAddr}' (${error})`, NS));
                }
            }
        },
    },
    ...
    },
    {
        ...
        exposes: [
            ...
            e
                ...
                .withDescription("Forcibly activating/deactivating smoke alarms. This command is only available " + "when Remote control permission is True."),
            e
