        ...
        if (found) {
            ...
            switch (found.type) {
                ...
                case ENUM:
                    ...
                    break;
                case TIME:
                    value = new Date(msg.data[key] * 1000).toLocaleString("fr-FR", {
    ...
    save_tic_elec_contract: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (msg.data.contractType !== undefined) {
                ...
                let contractType;
                if (node_buffer_1.Buffer.isBuffer(msg.data.contractType)) {
    ...
    for (const item of ticmeterDatas) {
        ...
        if (item.type === NUM_RW) {
            tz.convertSet = async (entity, key, value, meta) => {
                ...
                await entity.write(item.clust, 
                // @ts-expect-error too dynamic to type
                { [item.attr]: value }, { manufacturerCode: null });
            };
        }
        tzLocal.push(tz);
    }
...
exports.definitions = [
    {
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            endpoint.configuredReportings.forEach(async (r) => {
                await endpoint.configureReporting(r.cluster.name, reporting.payload(
                // @ts-expect-error dynamic, expected correct since already applied
                r.attribute.name, r.minimumReportInterval, 65535, r.reportableChange), { manufacturerCode: null });
            });
            for (const item of wanted) {
                ...
                reportingConfig.push(endpoint.configureReporting(item.clust, reporting.payload(
                // @ts-expect-error too dynamic to type
                item.attr, conf.min, conf.max, conf.change), { manufacturerCode: null }));
            }
        ...
        extend: [
            ...
            m.poll({
                ...
                poll: (device) => poll(device.getEndpoint(1), device).catch((error) => logger_1.logger.error(`Failed to poll '${device.ieeeAddr}' (${error})`, NS)),
            }),
        ],
